...
*****************************************************************************************/
    ...
    template <typename T> inline py::array_t<T> _row_to_arr(const arma::Row<T> * src, bool copy) {
        /* Convert armadillo col to numpy array */
        ssize_t tsize =  static_cast<ssize_t>(sizeof(T));
        ssize_t ncols = static_cast<ssize_t>(src->n_cols);

        const T * data = get_data<arma::Row<double>>(src, copy);
        py::capsule base = create_capsule(data);

        return py::array_t<T>(
            {static_cast<ssize_t>(1), ncols}, // shape
            {tsize, ncols * tsize}, // F-style contiguous strides
            data, // the data pointer
            base // numpy array references this parent
        );
    } /* _row_to_arr */
    ...
    template <typename T> inline py::array_t<T> _col_to_arr(const arma::Col<T> * src, bool copy) {
        /* Convert armadillo col to numpy array */
        ssize_t tsize =  static_cast<ssize_t>(sizeof(T));
        ssize_t nrows = static_cast<ssize_t>(src->n_rows);

        const T * data = get_data<arma::Col<double>>(src, copy);
        py::capsule base = create_capsule(data);
